...
(when (and (procedure? checker-tt+) (andmap struct? (list C1 C2 C3 C4)))
  (check-part 'a .5 (checker-tt+ C1 2) is (make-counter 1 2 0 '()))
  (check-part 'b .5 (checker-tt+ C5 8) is (make-counter 5 20 8 '((remus . 6) (vivi . 4))))
  )

(exercițiul 4 : 7.5 puncte)
(when (and (procedure? checker-et+) (andmap struct? (list C1 C2 C3 C4 C5)))
  (check-part 'a .5 (checker-et+ C1 2) is (make-counter 1 0 2 '()))
  (check-part 'b .5 (checker-et+ C5 8) is (make-counter 5 12 16 '((remus . 6) (vivi . 4))))
  )

(exercițiul 5 : 10 puncte)
(when (and (procedure? checker-add-to-counter) (andmap struct? (list C1 C2 C3 C4 C5)))
  (check-part 'a .25 (checker-add-to-counter C1 'ana 3) is (make-counter 1 3 3 '((ana . 3))))
  (check-part 'b .25 (checker-add-to-counter C5 'ana 3) is (make-counter 5 15 8 '((remus . 6) (vivi . 4) (ana . 3))))
  (check-part 'c .25 (checker-add-to-counter (checker-add-to-counter C5 'ana 3) 'bogdan 10)
              is (make-counter 5 25 8 '((remus . 6) (vivi . 4) (ana . 3) (bogdan . 10))))
  (check-part 'd .25 (checker-add-to-counter (checker-add-to-counter C3 'ana 3) 'bogdan 10)
              is (make-counter 3 13 3 '((ana . 3) (bogdan . 10))))
  )

(exercițiul 6 : 15 puncte)
(when (and (procedure? min-tt) (andmap struct? (list C1 C2 C3 C4)))
  (check-part 'a .125 (min-tt (list (counter 3 2 2 '(()))
                                   (counter 4 6 1 '((ana . 1) (geo 5))))) is '(3 . 2))
  (check-part 'b .125 (min-tt (list (counter 1 3 1 '((dan . 1) (lia . 2)))
                                    (counter 2 9 5 '((ana . 1) (geo . 3) (ema . 1)))
                                    (counter 3 2 2 '((leo . 2))))) is '(3 . 2))
  (check-part 'c .125 (min-tt (list (counter 1 12 4 '((mara . 4) (dan . 3) (gigi . 5)))
                                    (counter 2 9 4 '((florin . 3) (ana . 5)))
                                    (counter 3 4 4 '((mia . 4)))
                                    (counter 4 15 10 '((eliza . 1) (gabi . 3) (cristi . 2))))) is '(3 . 4))
  (check-part 'd .125 (min-tt (list (counter 1 12 4 '((mara . 4) (dan . 3) (gigi . 5)))
                                    (counter 2 10 8 '((florin . 8) (ana . 2)))
                                    (counter 3 10 4 '((mia . 2) (nicu . 6)))
                                    (counter 4 15 10 '((eliza . 1) (gabi . 3) (cristi . 2))))) is '(2 . 10))
  (check-part 'e .125 (min-et (list (counter 3 2 2 '(()))
                                    (counter 4 6 1 '((ana . 1) (geo 5))))) is '(4 . 1))
  (check-part 'f .125 (min-et (list (counter 1 3 1 '((dan . 1) (lia . 2)))
                                    (counter 2 9 5 '((ana . 1) (geo . 3) (ema . 1)))
                                    (counter 3 2 2 '((leo . 2))))) is '(1 . 1))
  (check-part 'g .125 (min-et (list (counter 1 12 4 '((mara . 4) (dan . 3) (gigi . 5)))
                                    (counter 2 9 4 '((florin . 3) (ana . 5)))
                                    (counter 3 4 4 '((mia . 4)))
                                    (counter 4 15 10 '((eliza . 1) (gabi . 3) (cristi . 2))))) is '(1 . 4))
  (check-part 'h .125 (min-et (list (counter 1 12 4 '((mara . 4) (dan . 3) (gigi . 5)))
                                    (counter 2 10 8 '((florin . 8) (ana . 2)))
                                    (counter 3 10 4 '((mia . 2) (nicu . 6)))
                                    (counter 4 15 10 '((eliza . 1) (gabi . 3) (cristi . 2))))) is '(1 . 4))
)
...
(when (andmap struct? (list C1 C2 C3 C4))
  ...
  (check-part 'l (/ 1. 16)
    (serve '((delay 1 3) (ana 2) (bogdan 3) (remove-first) (ensure 3) (cristi 6)
     ...
     (list
           ...
           (counter 3 0 0 '())
           (counter 4 0 0 '())))
  ; Teste care încearcă să detecteze diverse bug-uri.
  (check-part 'm (/ 1. 16)
    (serve '((ensure 6) (remove-first) (teo 7) (delay 4 19) (delay 3 18) (delay 4 5) (delay 4 3) (delay 2 7) )
           (list C1 C2 C3)
           (list C4)) is
     (list
           C1
           (counter 2 7 7 '())
           (counter 3 18 18 '())
           (counter 4 34 34 '((teo . 7)))
           ))
  (check-part 'n (/ 1. 16)
    (serve '((remove-first) (delay 1 17) (delay 5 6) (ana 10) (remove-first) (jan 1) (ensure 10) (geo 5) (ken 2) (remove-first) (ensure 12) (achim 5) (remove-first) (sia 2) (ralu 6) (delay 2 8) (delay 3 6) )
           (list C1 C2 C3)
           (list C4 (counter 5 0 0 '()))) is
     (list
           (counter 1 17 17 '())
           (counter 2 13 13 '((achim . 5)))
           (counter 3 11 11 '((geo . 5)))
           (counter 4 8 2 '((sia . 2) (ralu . 6)))
           (counter 5 6 6 '())
           ))
  (check-part 'o (/ 1. 16)
    (serve '((ensure 5) (remove-first) (ken 6) (jan 5) (ensure 12) (delay 4 3) (delay 5 12) (delay 1 2) (remove-first) (remove-first) (ensure 8) (ensure 11) (achim 4) )
           (list C1 C2)
           (list C3 C4 (counter 5 0 0 '()))) is
     (list
           (counter 1 4 4 '((achim . 4)))
           C2
           C3
           (counter 4 3 3 '())
           (counter 5 12 12 '())
  ...
  (check-part 'p (/ 1. 16)
    (serve '((bia 6) (paul 3) (nico 2) (remove-first) (dan 10) (cora 6) (ensure 8) (remove-first) (geo 8) (teo 10) (ensure 6) (delay 5 9) (jack 5) (vio 9) )
           (list C1)
           (list C2)) is
     (list
           (counter 1 5 5 '((jack . 5)))
           (counter 2 22 6 '((bia . 6) (dan . 10) (cora . 6)))
           (counter 3 18 8 '((geo . 8) (teo . 10)))
           (counter 4 9 9 '((vio . 9)))
           (counter 5 9 9 '())
           (counter 6 0 0 '())
           (counter 7 0 0 '())
